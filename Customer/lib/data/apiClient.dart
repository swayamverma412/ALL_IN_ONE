...
class ApiClient {
  ...
  Future<Http.Response> getData(String uri, bool header) async {
    ...
    try {
      ...
      if (header) {
        ...
        _response = await Http.get(Uri.parse(appBaseUrl + uri))
            .timeout(Duration(seconds: timeoutInSeconds));
      }
  ...
  Future<Http.Response> getData_Customeurl(String uri, bool header) async {
    Http.Response _response;
    try {
      ...
      if (header) {
        debugPrint('====> API Call in heder: $uri');
        await updateHeader();
        _response = await Http.get(Uri.parse(uri), headers: _mainHeaders)
            .timeout(Duration(seconds: timeoutInSeconds));
      } else {
        _response = await Http.get(Uri.parse(uri))
            .timeout(Duration(seconds: timeoutInSeconds));
      }
      print(_response.body.toString());
    } catch (e) {
  ...
  Future<Http.Response> postData(
    ...
    try {
      ...
      _response = await Http.post(Uri.parse(appBaseUrl + uri),
              body: b1, headers: _mainHeaders)
          .timeout(Duration(seconds: timeoutInSeconds));
...
*/
  ...
  Future<void> updateHeader() async {
    String? token = await SharedPreferenceClass.GetSharedData(AppContent.TOKEN);
    _mainHeaders = {
  ...
  Response handleResponse(Http.Response response, String uri) {
    ...
    Response _response = Response(
        _body != null ? _body : response.body, _body != null ? 200 : 1);
    ...
    if (_response.statusCode != 200 &&
      ...
      if (_response.body.toString().startsWith('{errors: [{code:')) {
        ...
        _response = Response(
            _errorResponse.errors![0].message.toString(), _response.statusCode);
