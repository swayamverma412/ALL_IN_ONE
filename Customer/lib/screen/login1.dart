...
import 'package:vendor_app/data/apiClient.dart';
...
class _LoginPage1State extends State<LoginPage1> {
  ...
  void verify(dynamic para) async {

    try {
     var response=await ApiClient(appBaseUrl: AppContent.BASE_URL).postData(AppContent.login,para);
      if (response.statusCode == 200) {
        var data=jsonDecode(response.body);
        await SharedPreferenceClass.SetSharedData(AppContent.TOKEN,data["Token"].toString());
        await SharedPreferenceClass.SetSharedData(AppContent.CUSTOMERID,data["customerId"].toString());
        print("verifed!");
      } else {
        print("Login failed");
      }
