...
class authRepo{
 ...
 /* Future<Http.Response> Updateprofilr(String id,Map<String,String> body) async {
  ...
  Future<Http.Response> Updateprofilr(String id, Map<String, String> signUpBody,List<MultipartBody> list) async {
    return await apiClient.putMultipartData(AppContent.updateStore+id, signUpBody, list);
    //  return await apiClient.postDataSecond(AppContent.signup, signUpBody);
  }
  Future<Http.Response> generateQr(Map<String, String> LoginBody) async {
    String id= await SharedPreferenceClass.GetSharedData(AppContent.CUSTOMERID);
    return await apiClient.postData(AppContent.generateQr+id, LoginBody);
  }
